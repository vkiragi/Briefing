...
class BaseSportsFetcher:
    ...
    def fetch_scores(self, sport: str, limit: int = 10, date: Optional[str] = None) -> List[Dict]:
        ...
        """
        # Tennis uses a different API endpoint
        if sport.lower().startswith('tennis'):
            return self._fetch_tennis_scores(sport, limit)
    ...
    def _fetch_tennis_scores(self, sport: str, limit: int = 10) -> List[Dict]:
        """
                ...
                for league_data in sport_data.get('leagues', []):
                    ...
                    for event in events:
                        ...
                        game_info = {
                            ...
                            'set_scores': home_score if state != 'pre' else None,  # Full score string
                        }
    ...
    def fetch_schedule(self, sport: str, limit: int = 10, date: Optional[str] = None) -> List[Dict]:
        ...
        """
        # Tennis uses a different approach - show upcoming tournaments
        if sport.lower().startswith('tennis'):
    ...
    def fetch_live(self, sport: str, limit: int = 10) -> List[Dict]:
        ...
        """
        # Tennis uses a different API - filter for live matches from header endpoint
        if sport.lower().startswith('tennis'):
            matches = self._fetch_tennis_scores(sport, limit)
